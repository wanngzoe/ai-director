    ...
    <div className="dashboard-page">
      <main className="dashboard-content">
        ...
        <div className="script-tabs">
          ...
          <button className={`script-tab ${activeTab === 'custom' ? 'active' : ''}`} onClick={() => setActiveTab('custom')}>AI自定义修改</button>
...
支持长文本自动滚动..."
            ...
            <div className="ai-assistant-panel">
              ...
              <div className="ai-tools-grid">
                ...
                <div className="ai-tool-card" onClick={() => setActiveTab('custom')} style={{ border: '2px dashed var(--primary)', background: 'rgba(99,102,241,0.1)' }}>
                  <h4>AI自定义修改</h4>
                  <p>输入修改意见，AI帮你改</p>
                </div>
              </div>
        ...
        {activeTab === 'custom' && (
          <div className="script-editor-container">
            {/* Opening Refinement Mode */}
            <div className="script-editor-main" style={{ flex: 1.2 }}>
              <div className="script-toolbar">
                <button className="btn btn-sm btn-primary"><Wand2 size={14} /> 开始精修</button>
                <button className="btn btn-sm btn-outline">预设模板</button>
              </div>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20, height: '100%' }}>
                {/* Original Script */}
                <div style={{ display: 'flex', flexDirection: 'column' }}>
                  <div style={{ fontSize: 13, fontWeight: 500, marginBottom: 8, color: 'var(--text-muted)' }}>原剧本（开头部分）</div>
                  <textarea
                    style={{
                      flex: 1, padding: 16, background: 'var(--bg-card)', border: '1px solid var(--border)',
                      borderRadius: 8, color: 'var(--text)', fontSize: 14, lineHeight: 1.8, resize: 'none'
                    }}
                    placeholder="请输入或粘贴剧本开头内容...
...
林婉儿站在落地窗前，手握文件..."
                    value={scriptText || sampleScript.slice(0, 300)}
                  />
                </div>

                {/* Refined Result */}
                <div style={{ display: 'flex', flexDirection: 'column' }}>
                  <div style={{ fontSize: 13, fontWeight: 500, marginBottom: 8, color: 'var(--text-muted)' }}>精修结果</div>
                  <textarea
                    style={{
                      flex: 1, padding: 16, background: 'var(--bg-card)', border: '1px solid var(--border)',
                      borderRadius: 8, color: 'var(--text)', fontSize: 14, lineHeight: 1.8, resize: 'none',
                      borderColor: customResult ? 'var(--primary)' : 'var(--border)'
                    }}
                    placeholder="AI精修后的开头将显示在这里...

点击「开始精修」按钮，AI将为您生成更抓人的剧本开头。"
                    value={customResult}
            ...
            <div className="ai-assistant-panel">
              ...
              <div style={{ marginBottom: 20 }}>
                ...
                <textarea
                  style={{
                    width: '100%', minHeight: 100, padding: 14, background: 'var(--bg-card)',
...
- 把节奏改得更快一些"
              ...
              <div className="next-steps">
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                  <div
                    style={{ padding: 12, background: 'var(--bg-input)', borderRadius: 8, cursor: 'pointer' }}
                    onClick={() => setCustomPrompt('增加悬念感，让读者想知道后续发生了什么')}
                  >
                    <div style={{ fontSize: 13, fontWeight: 500 }}>增加悬念</div>
                    <div style={{ fontSize: 12, color: 'var(--text-muted)' }}>让开头更具吸引力</div>
                  </div>
                  <div
                    style={{ padding: 12, background: 'var(--bg-input)', borderRadius: 8, cursor: 'pointer' }}
                    onClick={() => setCustomPrompt('加快节奏，删除冗余描述，保持紧张感')}
                  >
                    <div style={{ fontSize: 13, fontWeight: 500 }}>加快节奏</div>
                    <div style={{ fontSize: 12, color: 'var(--text-muted)' }}>删除拖沓内容</div>
                  </div>
                  <div
                    style={{ padding: 12, background: 'var(--bg-input)', borderRadius: 8, cursor: 'pointer' }}
                    onClick={() => setCustomPrompt('加入更多人物心理描写，让角色更立体')}
                  >
                    <div style={{ fontSize: 13, fontWeight: 500 }}>心理描写</div>
                    <div style={{ fontSize: 12, color: 'var(--text-muted)' }}>丰富角色内心</div>
                  </div>
                </div>
              </div>
            </div>
